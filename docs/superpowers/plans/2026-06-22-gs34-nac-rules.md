...
- [ ] **Step 3: Add the check class** — append to `src/digital_twin/checks/nac/shadowing.py`. Add exactly the imports the snippet below uses (no others — `ruff` fails on unused): `Finding`, `FindingCategory`, `FindingSource`, `Severity`, `Cause`, `ObjectRef` (from `digital_twin.contracts`); `Confidence`, `ConfidenceLevel`, `IRDiff` (from `digital_twin.ir`); `CheckContext`, `CheckResult`, `Status`, `Coverage`, `CoverageState` (from `digital_twin.checks.base`). `requires()` returns `frozenset()`, so **no** capability import is needed:
...
4. Build `baseline_raw: dict[id, raw]` from `fetch.rules` **first-wins** (skip a row whose id is already present — NOT a dict comprehension, which is last-wins) so it matches `build_nac_ir`'s dedup; id-less rows skipped here (they surface via `base_ir`'s ingest).
