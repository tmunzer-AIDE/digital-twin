...
def test_unparseable_order_is_none():
    ir, _ = build_nac_ir([_rule(order="abc")], [])
    assert ir.nacrules[0].order is None
...
```python
from digital_twin.contracts import parse_change_plan
...
(Match the existing import style for `mistapi`, `datetime`/`UTC`, `Confidence`, `FindingSource`, etc. — copy from the surrounding file. Add `resolve_org_nac` to any recording/decorator provider wrapper used in the drivers, mirroring how `resolve_org_template` is wrapped.)
...
git commit -m "feat(gs34): provider resolve_org_nac + NacFetch (asymmetric failure)"
```
...
- [ ] **Step 5: Add MCP routing** — in `src/digital_twin/drivers/mcp_server.py` `simulate_change`, **before** the `if _is_org_plan(...)` block:
...
(Import `_is_org_nac_plan` from cli or define a shared predicate; import `simulate_org_nac`, `org_nac_verdict_to_dict`.)
