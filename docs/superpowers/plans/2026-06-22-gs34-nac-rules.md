...
- [ ] **Step 5: Note the canonical refresh** — in `tools/extract_oas.py`, add to `WANTED` (so a future `extract_oas` run regenerates it from the live OpenAPI):
...
def _plan(action="update", payload=None, site_id=None):
    return parse_change_plan({
        ...
        "ops": [{"action": action, "order": 0, "object_type": "nacrule",
                 "object_id": "r1", "payload": payload or {"name": "x"}}],
    })
...
def test_enabled_flip_is_a_cause():
    ...
    res = _run(base, prop)
    f = next(f for f in res.findings if f.code.endswith("introduced"))
    assert f.subject.id == "b" and any(c.ref.id == "a" for c in f.caused_by)
...
6. `base_ir, base_findings = build_nac_ir(baseline_raw.values(), fetch.tags)`; `prop_ir, prop_findings = build_nac_ir(proposed_raw.values(), fetch.tags)`.
7. `adapter_findings += prop_findings + fetch.tag_findings`.
...
def simulate_org_nac(
    ...
    *,
    ...
    base_ir, _ = build_nac_ir(baseline_raw.values(), fetch.tags)
    prop_ir, prop_findings = build_nac_ir(proposed_raw.values(), fetch.tags)
    adapter_findings += prop_findings + fetch.tag_findings
...
**Note:** This is the empirical check on the conservative semantics. Capture the 13 rules' modeled fields (order/enabled/action/matching surface) from the live org — redact all UUIDs to stable synthetic ids per the replay redaction manifest. Compute shadows on the *proposed == baseline* state (so all shadows are pre-existing INFO) AND on a perturbed state (move one broad rule earlier) and assert the introduced set matches a hand-checked expectation.
