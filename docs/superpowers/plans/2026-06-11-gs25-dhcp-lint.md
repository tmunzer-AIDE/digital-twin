...
class DhcpScopeLintCheck:
    ...
    def run(self, ctx: CheckContext) -> CheckResult:
        ...
        # --- abstention, with DIMENSION-SPECIFIC relevance (review P2 r2):
        # range blindness is PAIRWISE — any scope change cannot be cleared
        # against an unreadable row, so any dhcp_scope delta makes it
        # relevant. Subnet blindness is PER-SCOPE — it can only hide a
        # violation of THAT scope, so it notes only when that scope itself
        # changed (an unchanged blind gateway scope elsewhere must not
        # PARTIAL-floor an unrelated scope edit).
        conclusions = [f for f in findings if f.severity is not Severity.INFO]
        scopes_touched = ctx.diff.touches("dhcp_scope")
        changed_ids = {
            r.id
        ...
        range_notes = tuple(
            ...
            for sid, s in sorted(prop.items())
            if _range(s) is None
        ) if (conclusions or scopes_touched) else ()
        subnet_notes = tuple(
            f"scope {sid}: owning network subnet is declared-but-unreadable or "
            "unknowable — subnet coherence is unevaluated for it"
            for sid, s in sorted(prop.items())
            if s.subnet_unresolved and sid in changed_ids
        )
        notes = range_notes + subnet_notes
...
def test_baseline_parity_folds_with_the_baseline_vc_map():
    ...
    def ir_vc(folded):
        b = IRBuilder()
        b.add_device(replace(sw("R", vc_members=("S",) if folded else ()),))
...
def _egress_trust(
    ...
    ir: IR,
    vc_root: Mapping[str, str],  # vc_root_map(ir), computed ONCE per run()
    did: str,
    vlan: int,
    source: str,
):
    ...
    Candidates AND reachability both come from the VLAN graph (review P1
    round 2): an edge is a candidate only if it actually CARRIES the vlan —
    deriving candidates from raw ir.links + a local-port carriage test can
    false-SAFE when the local port carries V but the edge does not (the peer
    end drops it). Local egress ports come from the edge payload's
    member_ports. An edge whose carriage confidence is below HIGH yields
    trust UNKNOWN — uncertain carriage must never produce a confident ok OR
    blocked. `did`/`source` are VC-FOLDED node ids (node_for/vc_root_map).
    """
    ...
    for _, neighbor, data in g.edges(did, data=True):
        edge = data["data"]  # L2Edge: vlans / member_ports / confidence
        reaches = neighbor == source or (
            neighbor in pruned and source in pruned
            and nx.has_path(pruned, neighbor, source)
        )
        if not reaches:
            continue
        ...
        for pid in edge.member_ports:
            ...
            if node_for(vc_root, port.device_id) != did:
                continue  # the peer side's member port
    ...
    def run(self, ctx: CheckContext) -> CheckResult:
        ...
        for did, dev in sorted(prop_ir.devices.items()):
            if dev.role is not DeviceRole.SWITCH:
                continue
            node = node_for(prop_root, did)  # graph nodes are VC-folded
            for vlan in _snooped_vlans(prop_ir, did):
                ...
                for source in sources:
                    ...
                    if source == "site" or src_node == node:
                        continue  # unlocatable (noted) / local (folded-id compare)
                    ...
                    pre = (
                        ...
                        and _egress_trust(
                            # baseline graph nodes fold by the BASELINE VC map —
                            # reusing proposed-folded ids can mis-demote when
                            # VC membership itself changed (review r3)
                            ctx.baseline, base_ir, base_root,
                            node_for(base_root, did), vlan,
