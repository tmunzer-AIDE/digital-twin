...
def test_gs25_dhcp_lint_leaves_in_scope():
    ...
    assert screen_op("site_setting", cur, eff).allowed
    ...
    assert screen_op("device", cur_d, eff_d).allowed
...
class DhcpScopeLintCheck:
    ...
    def run(self, ctx: CheckContext) -> CheckResult:
        ...
        return CheckResult(
            ...
            confidence=_HIGH if findings or not notes else min_confidence(
                _HIGH, Confidence(level=ConfidenceLevel.MEDIUM,
                                  reasons=("unreadable scope ranges",))
            ),
...
class DhcpSnoopingCheck:
    ...
    def run(self, ctx: CheckContext) -> CheckResult:
        ...
        return CheckResult(
            ...
            confidence=(
                min(  # weakest finding confidence, else HIGH
                    (f.confidence for f in findings),
                    key=lambda c: c.level is ConfidenceLevel.HIGH,
                    default=_HIGH,
                )
                if findings else (_HIGH if evaluated or not notes else None)
            ),
