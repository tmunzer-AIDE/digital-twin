...
**As-built (2026-06-25):** four decisions made during the build SUPERSEDE this plan — see §12 of the design spec. In brief: (1) the jsonschema validator strips `additionalProperties:false` (`_strip_closed`) and the walker reads the faithful closed schema (`_raw_schema`) as the sole closedness owner; (2) the walker is scoped by a SEPARATE `unknown_scope_roots` (roots with actual value deltas via `changed_paths`), NOT `scope_roots`/`_changed_roots` — full-object PUTs made the all-roots scope re-audit pre-existing fields (top-level + nested `mist_nac.*`), so §8's "pre-existing in untouched root" promise is dropped; (3) device server-managed roots are skipped via `_SERVER_MANAGED_ROOTS_BY_TYPE` (device-scoped), keeping the global `IGNORED_RAW_FIELDS` minimal; (4) OAS refreshed from `mistsys/mist_openapi`, with `switch_bgp_config_neighbor.disabled` patched locally pending upstream.
