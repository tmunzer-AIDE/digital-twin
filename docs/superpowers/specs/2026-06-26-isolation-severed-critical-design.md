...
- **`ERROR`** when `high` but `lost_anchor_nodes` is empty (exit-less domain; the delta removed the only exit; or the far-side anchor is *newly added* by the delta so there was no baseline reach to lose — the conservative direction where we must not invent CRITICAL).
...
where, reusing the `anchors = exit_anchor_nodes(ctx.proposed.ir)` already computed in `run()` and adding the baseline anchor set:
...
`lost_anchor_nodes` is the set of nodes that (a) sit on the *other side* of the cut (`baseline_home - fragment`), (b) are an exit anchor in **proposed** (`& anchors` — the gateway *survives* the delta), and (c) were an exit anchor in **baseline** (`& baseline_anchors` — the fragment actually *had* reach to it before). All three are required to be the true twin of `blackhole.exit_lost`, which fires only on a **baseline** exit that is no longer reached. Dropping (c) would mis-escalate a delta that severs the fragment while *adding* a brand-new exit on the far side (no baseline gateway to lose) — that stays ERROR. The `(baseline_home - fragment)` form is written explicitly (rather than the today-equivalent `baseline_home & anchors & baseline_anchors`, equivalent only because the existing `if fragment & anchors: continue` suppression means a flagged fragment never holds a proposed anchor) so it stays correct if that suppression ever changes.

> The suppression itself (`if fragment & anchors: continue`) stays **proposed-only**: a fragment that retains *any* proposed exit is not L2-isolated regardless of baseline. Only the CRITICAL predicate needs the both-states intersection.
...
- **ERROR — newly-added far-side anchor:** the delta severs the fragment at HIGH confidence *and* adds a brand-new IRB/gateway on the far side (anchor present in proposed, absent in baseline) → `severity is Severity.ERROR`, `evidence["lost_anchor_nodes"] == []` (the fragment had no baseline reach to lose), even though `evidence["exit_anchor_nodes"]` lists the new proposed anchor. Guards the `blackhole.exit_lost` twin semantics.
